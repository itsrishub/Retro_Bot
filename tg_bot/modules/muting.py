...
def mute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if member:
        if is_user_admin(chat, user_id, member=member):
            ...
            message.reply_text("Mute key applied! 🤐")
            return "<b>{}:</b>" \
                   "\n#MUTE" \
                   "\n<b>Admin:</b> {}" \
...
def unmute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if member.status != 'kicked' and member.status != 'left':
        if member.can_send_messages and member.can_send_media_messages \
            ...
            message.reply_text("Unmute key applied!")
            return "<b>{}:</b>" \
                   "\n#UNMUTE" \
                   "\n<b>Admin:</b> {}" \
                   "\n<b>User:</b> {}".format(html.escape(chat.title),
...
def temp_mute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        if member.can_send_messages is None or member.can_send_messages:
            ...
            message.reply_text("shut up! 😠 Mute key applied for {}!".format(time_val))
            return log
        else:
            message.reply_text("This user is already muted.")
        ...
        if excp.message == "Reply message not found":
            # Do not reply
            message.reply_text("shut up! 😠 Muted key applied for {}!".format(time_val), quote=False)
            return log
        else:
            LOGGER.warning(update)
            LOGGER.exception("ERROR muting user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text("Well damn, I can't apply mute key to that user.")
